...
check_free_space_func() {
	...
	if [[ $current_free_space -lt $space_need_megabytes ]]; then
		printf " - ${RC}FAIL${NC}"
		exit 1
	else
		printf " - ${GC}OK${NC}\n"
...
nginx_compilation_args_func() {
	...
	local nginx_compilation_pre
	nginx_compilation_pre=$(2>&1 nginx -V | sed 's|^configure arguments.*||gi')
	local nginx_compilation_args
	nginx_compilation_args=$(2>&1 nginx -V | grep -i 'configure arguments:' | sed -E 's|^configure arguments:(.*)|\1|gi' | sed 's@ --@\n--@gi')
...
check_exit_code() {
	if test $EXIT_STATUS -ne 0; then
		printf "\n\n${RC}ERROR - last command not succeeded ( check $NGX_RECOMPILE_LOG_FILE )${NC}\n"
		exit 1
	fi
}
...
ngx_check_dynamic_modules_func() {
	...
	for ngx_path in "${NGX_PATHS[@]}"; do
		if [[ -d "$ngx_path" ]]; then
			local files
			files=$(grep -RiIl 'load_module' "$ngx_path" --include="*.conf" 2>/dev/null | grep -v '#load_module')
			if [[ -n "$files" ]]; then
				echo "$files" | xargs sed -i 's@load_module@#load_module@gi' >> "$NGX_RECOMPILE_LOG_FILE" 2>&1
				mapfile -t new_files < <(echo "$files")
				COMMENTED_FILES+=("${new_files[@]}")
...
build_brotli_func() {
	...
	local CMAKE_VERSION
	CMAKE_VERSION=$(cmake --version 2>/dev/null | grep -o -P '\d+\.?\d+\.?\d+' | head -n1)
...
install_other_staff_func() {
	cd "$SRC_DIR" || {
		echo "ERROR: Failed to cd to $SRC_DIR" >> "$NGX_RECOMPILE_LOG_FILE"
		exit 1
	}
	
	{
		...
		echo "############################################"
		echo "CLONING NGINX ZSTD MODULE"
		echo "############################################"
		git clone https://github.com/tokers/zstd-nginx-module.git "$SRC_DIR/ngx_http_zstd_module" || {
			echo "ERROR: Failed to clone zstd module" >> "$NGX_RECOMPILE_LOG_FILE"
		}
		...
		git clone --recursive https://github.com/google/boringssl.git || {
			echo "ERROR: Failed to clone boringssl" >> "$NGX_RECOMPILE_LOG_FILE"
		}
		...
		git clone https://github.com/leev/ngx_http_geoip2_module.git || {
			echo "ERROR: Failed to clone geoip2" >> "$NGX_RECOMPILE_LOG_FILE"
		}
		...
		git clone https://github.com/openresty/headers-more-nginx-module.git || {
			echo "ERROR: Failed to clone headers_more" >> "$NGX_RECOMPILE_LOG_FILE"
		}
		...
		git clone https://github.com/wandenberg/nginx-push-stream-module.git || {
			echo "ERROR: Failed to clone push_stream" >> "$NGX_RECOMPILE_LOG_FILE"
		}
...
ngx_compilation_default_func() {
	...
	# Check if zstd module is needed
	local zstd_configure=""
	if nginx -T 2>&1 | grep -qi "zstd"; then
		zstd_configure="--add-module=$SRC_DIR/ngx_http_zstd_module"
	fi
	...
	for nginx_module in "${nginx_modules_array[@]}"; do
		case "$nginx_module" in
			...
			*substitutions_filter*)
				...
				;;
			*zstd*)
				zstd_module_configure_string="--add-module=$SRC_DIR/ngx_http_zstd_module"
				;;
			*lua_filter*)
				cd "$SRC_DIR/lua-resty-core" || {
				echo "ERROR: Failed to cd to $SRC_DIR/lua-resty-core" >> $NGX_RECOMPILE_LOG_FILE
...
EOF
	...
	local src_rpm
	src_rpm=$(find . -maxdepth 1 -type f -name 'bx-nginx-*.src.rpm' | head -n1)
	...
	local rpm_files=()
	for rpm_file in ./bx-nginx-*.rpm; do
	    [[ -f "$rpm_file" ]] || continue
	    [[ "$rpm_file" != *debug* ]] && rpm_files+=("$rpm_file")
	done
	
	if [[ ${#rpm_files[@]} -eq 0 ]]; then
	    printf "\n${RC}No bx-nginx RPM files found${NC}\n"
	    echo "ERROR: No bx-nginx RPM files found" >> "$NGX_RECOMPILE_LOG_FILE"
	    return 1
	fi
	
	rpm -Uvh "${rpm_files[@]}" >> "$NGX_RECOMPILE_LOG_FILE" 2>&1 || {
